...
if is_admin:
    ...
    if all_projects.empty:
        ...
        # Detected brands from AI responses (no cache)
        ...
        if pending_rows:
            ...
            if new_pending:
                edit_df = pd.concat(
                    [edit_df, pd.DataFrame(new_pending)], ignore_index=True
                )

        # -----------------------------------------------------------------------
        # SECTION A — Saved brands
        # -----------------------------------------------------------------------
        st.write("#### Saved brands")

        search_a = st.text_input("Search brands", key=f"search_a_{brand_proj_id}", placeholder="Filter by name…")
        ...
        edited_a = st.data_editor(
            ...
            use_container_width=True,
            hide_index=True,
            key=f"brand_editor_a_{brand_proj_id}_{search_a}_{len(pending_rows)}",
        )
        st.caption(
        ...
        # -----------------------------------------------------------------------
        # SECTION B — Suggested brands (from AI responses)
        # -----------------------------------------------------------------------
        st.write("#### Suggested brands (from AI responses)")
        ...
        if detected_df.empty:
            ...
            if sugg_df.empty:
                ...
                edited_b = st.data_editor(
                    ...
                    use_container_width=True,
                    hide_index=True,
                    key=f"brand_editor_b_{brand_proj_id}_{search_b}",
                )
        ...
        if st.button("Salva brand", type="primary", key="save_brands_btn"):
            # Merge visible edited rows with hidden (non-matching search) rows
